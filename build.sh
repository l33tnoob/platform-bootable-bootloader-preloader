...
function build_preloader () {
    ...
    #source custom/${MTK_PROJECT}/${MTK_PROJECT}.mk
    ...
    #MTK_ROOT_CUSTOM=../../../integrate/mediatek/custom/out
    MTK_ROOT_CUSTOM=custom

    echo "[PRELOADER_OUT]=${PRELOADER_OUT}"

    ##############################################################
    # Binary Generation
    #

    make PRELOADER_OUT=${PRELOADER_OUT} ${FORCE_CFG}

    if [ ! -f "${PL_IMAGE}" ]; then echo "BUILD FAIL."; exit 1; fi

    PL_FUN_MAP=${PRELOADER_OUT}/function.map

    source platform/`echo ${MTK_PLATFORM} | tr [A-Z] [a-z]`/check_size.sh
...
function post_process () {
    ...
    PBP_TOOL=tools/pbp
    MTK_PATH_PLATFORM=platform/`echo ${MTK_PLATFORM} | tr [A-Z] [a-z]`/
    CUSTOM_PATH=${MTK_ROOT_CUSTOM}/${TARGET}/security
    #CUSTOM_PATH=${MTK_ROOT_CUSTOM}
