...
static void msdc_dump_clock_sts(struct mmc_host *host)
{
	...
	printf(" CLK_CFG_2       [0x%p][bit[31:24]should be 0x01]=0x%x", CLK_CFG_2,MSDC_READ32(CLK_CFG_2));
	printf(" CLK_CFG_3       [0x%p][bit[15:0]should be 0x0202]=0x%x", CLK_CFG_3,MSDC_READ32(CLK_CFG_3));
	printf(" PERI_PDN_STA0   [0x%p][bit13=msdc0, bit14=msdc1,0:on,1:off]=0x%x",PERI_PDN_STA0,MSDC_READ32(PERI_PDN_STA0));
#endif
}
...
void msdc_config_clock(struct mmc_host *host, int ddr, u32 hz, u32 hs_timing)
{
    ...
    MSDC_SET_FIELD(MSDC_CFG, MSDC_CFG_CKMOD_HS400 | MSDC_CFG_CKMOD | MSDC_CFG_CKDIV, (hs400_src << 14) | (mode << 12) | div);
...
void msdc_set_host_pwr(struct mmc_host *host, int on)
{
    ...
    switch(host->id){
        ...
        default:
         ...
         printf("[MSDC]config VEMC to 3V in preloader\n");
     #endif
     #if defined(MMC_MSDC_DRV_LK)
